...
- `PATCH /api/v1/projects/:projectID/models/:modelID/feature-dsl-graph` accepts a complete validated `.lcad.json` source plus `expected_revision`. It must preserve the parameter envelope, create an immutable model revision, update occurrence revision bindings, and append one reversible `feature-graph-change` History entry with stable top-level node transitions atomically.
