...
- For multi-source preview, load assets into a scene group and frame the combined bounds. The CAD document schema v2 flat assembly owns occurrence revision bindings and placements used by preview and selected STEP compound download; do not imply nested assemblies, mates, source STEP product-structure preservation, or cross-model merge semantics.
