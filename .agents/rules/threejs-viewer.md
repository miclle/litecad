...
- Do not render hard-coded demo geometry as project model data. The public landing page may use an explicitly labeled, self-contained sample model, but the project workbench may show only neutral grid/axis/empty-state context or project-owned source and derived preview geometry such as browser-kernel meshes, OBJ, self-contained GLTF, or GLB assets.
...
- For multi-source preview, load assets into a scene group and frame the combined bounds. The CAD document schema v4 assembly owns occurrence identity, name, order, parent group, suppression, revision binding, and placement used by preview and selected STEP compound download. Multiple occurrences may reference one source model; direct suppression or suppression by any ancestor group keeps an occurrence durable but excludes it from the scene and export. Reusable-assembly instances deliberately expand into ordinary revision-pinned occurrences so the existing preview/export pipeline remains authoritative; their member transforms are read-only and only the tagged ancestor group may be suppressed. `point-coincident-v1` mates resolve driven translation before scene composition; hide the transform control for a solver-owned driven occurrence or reusable-instance member. These snapshot and point-translation semantics do not imply live/nested/cross-project reusable documents, rotational/general mechanical solving, source STEP product-structure preservation, or cross-model merge semantics.
