...
- For multi-source preview, load assets into a scene group and frame the combined bounds. The CAD document schema v2 flat assembly owns occurrence identity, name, order, suppression, revision binding, and placement used by preview and selected STEP compound download. Multiple occurrences may reference one source model; suppressed occurrences stay durable but do not enter the scene or export. Do not imply nested assemblies, mates, source STEP product-structure preservation, or cross-model merge semantics.
