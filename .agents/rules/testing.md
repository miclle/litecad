...
- Keep the landing-page interactive preview/WebGL fallback, shell, import, transform conflict/Undo/Redo, occurrence/group/existing-position-link/reusable-subassembly, inspection/section, Assistant/parameter persistence, and export workflows in independent specs so failures identify the affected product path.
...
- Run `task test-browser` for landing-page preview, route, workbench panel, browser CAD worker, and visible interaction changes.
