            ...
            {entries.map((entry) => (
              <div className="flex items-start gap-3 rounded px-2 py-2.5 hover:bg-[#f8fafc]" key={entry.id}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {entry.feature_graph_version ? (
                    <p className="mt-1 font-mono text-[10px] text-[#64748b]">
                      {t('project.history.featureGraphVersion', { version: entry.feature_graph_version })}
                    </p>
                  ) : null}
                  {entry.feature_graph_transitions && entry.feature_graph_transitions.length > 0 ? (
                    <ul aria-label={t('project.history.featureGraphChanges')} className="mt-1.5 space-y-1 border-l border-[#dbe3ec] pl-2">
                      {entry.feature_graph_transitions.map((transition) => {
                        const path = transition.after_path || transition.before_path || transition.node_id
                        return (
                          <li
                            className="truncate font-mono text-[10px] text-[#475569]"
                            key={`${transition.node_id}:${transition.change}:${transition.before_path ?? ''}:${transition.after_path ?? ''}`}
                            title={path}
                          >
                            {path} · {t(`project.history.featureGraphChange.${transition.change}`)}
                          </li>
                        )
                      })}
                    </ul>
