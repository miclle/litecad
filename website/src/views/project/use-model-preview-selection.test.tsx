...
describe('useModelPreviewSelection', () => {
  it('keeps live callbacks, selection, visibility, and translations synchronized without rebuilding the scene', () => {
    ...
    const { result, rerender } = renderHook(
      ({ selectedNodeId, visibleModelIds }) =>
      ...
      { initialProps: { selectedNodeId: 'node_one', visibleModelIds: ['model_one'] as readonly string[] } },
    )

    expect(result.current.selectedNodeIdRef.current).toBe('node_one')
    expect(result.current.visibleModelIdsRef.current).toEqual(['model_one'])

    act(() => rerender({ selectedNodeId: 'node_two', visibleModelIds: [] }))
