...
describe('useProjectWorkbenchParametricModelCommands', () => {
  ...
  it('saves a generated artifact as a model and refreshes dependent project queries', async () => {
    ...
    mockedUpdateProjectParametricArtifact.mockResolvedValue({
      data: { artifact: artifact() },
    } as Awaited<ReturnType<typeof updateProjectParametricArtifact>>)
    mockedSaveProjectParametricArtifactModel.mockResolvedValue({
      data: { model },
    } as Awaited<ReturnType<typeof saveProjectParametricArtifactModel>>)
    ...
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['projects', 'prj_commands', 'models'],
    })
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['projects', 'prj_commands', 'cad-document'],
    })
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['projects', 'prj_commands', 'parametric-artifacts'],
    })
  })
  ...
  it('applies a generated artifact to an existing LiteCAD model revision', async () => {
    ...
    mockedUpdateProjectFeatureDSLGraph.mockResolvedValue({
      data: { model },
    } as Awaited<ReturnType<typeof updateProjectFeatureDSLGraph>>)
    const queryClient = new QueryClient()
    ...
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['projects', 'prj_commands', 'models', 'model_revision', 'revisions'],
    })
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['projects', 'prj_commands', 'cad-document'],
    })
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['projects', 'prj_commands', 'cad-document', 'history'],
    })
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['projects', 'prj_commands', 'parametric-artifacts'],
    })
