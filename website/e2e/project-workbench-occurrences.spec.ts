...
test('authors repeated assembly occurrences through reload, history, preview, and export selection', async ({ page }) => {
	...
	const draggedOccurrenceOption = page.getByRole('option', { name: /Fixture right/ })
	const targetOccurrenceOption = page.getByRole('option', { name: /Smoke bracket/ })
	fixture.state.occurrenceMoveDelayMs = 3_000
	await expect(draggedOccurrenceOption).toHaveAttribute('aria-disabled', 'false')
	const draggedOccurrence = await draggedOccurrenceOption.boundingBox()
	const draggedOccurrenceRow = await draggedOccurrenceOption.evaluate((element) => {
		const row = element.closest('[data-occurrence-row]')
		if (!row) throw new Error('Sortable occurrence row is missing')
		const rect = row.getBoundingClientRect()
		return { height: rect.height, width: rect.width, x: rect.x, y: rect.y }
	})
	const targetOccurrence = await targetOccurrenceOption.boundingBox()
	expect(draggedOccurrence).not.toBeNull()
	expect(targetOccurrence).not.toBeNull()
	await page.mouse.move(draggedOccurrence!.x + draggedOccurrence!.width / 2, draggedOccurrence!.y + draggedOccurrence!.height / 2)
	await page.mouse.down()
	await page.mouse.move(draggedOccurrence!.x + draggedOccurrence!.width / 2, draggedOccurrence!.y + draggedOccurrence!.height / 2 - 8)
	const dragOverlay = page.locator('.shadow-lg').filter({ hasText: 'Fixture right' })
	await expect(dragOverlay).toBeVisible()
	await expect(dragOverlay.locator('.lucide-eye')).toBeVisible()
	await expect(dragOverlay.locator('.lucide-file-text')).toHaveCount(0)
	const dragOverlayBox = await dragOverlay.boundingBox()
	expect(dragOverlayBox).not.toBeNull()
	expect(Math.abs(dragOverlayBox!.x - draggedOccurrenceRow.x)).toBeLessThan(2)
	expect(Math.abs(dragOverlayBox!.y - draggedOccurrenceRow.y)).toBeLessThan(2)
	expect(Math.abs(dragOverlayBox!.width - draggedOccurrenceRow.width)).toBeLessThan(2)
	expect(Math.abs(dragOverlayBox!.height - draggedOccurrenceRow.height)).toBeLessThan(2)
	await page.mouse.move(targetOccurrence!.x + targetOccurrence!.width / 2, targetOccurrence!.y + targetOccurrence!.height / 2, { steps: 8 })
	await page.mouse.up()
	expect(fixture.state.occurrenceMoveCount).toBe(0)
	const droppedOccurrence = await draggedOccurrenceOption.boundingBox()
	const targetAfterDrop = await targetOccurrenceOption.boundingBox()
	expect(droppedOccurrence).not.toBeNull()
	expect(targetAfterDrop).not.toBeNull()
	expect(droppedOccurrence!.y).toBeLessThan(targetAfterDrop!.y)
