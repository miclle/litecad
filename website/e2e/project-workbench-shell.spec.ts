...
test('persists derived measurements and browser-kernel section geometry', async ({ page }) => {
  ...
  const analysisRecordsButton = page.getByRole('button', { name: 'Analysis', exact: true })
  await expect(analysisRecordsButton).toHaveText('Analysis')
  ...
  await expect(page.getByText('Generation 1 · Superseded')).toBeHidden()
  await page.getByRole('button', { name: 'History 3' }).click()
