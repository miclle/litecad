...
Do not document or implement AI-driven geometry mutation, AI tool orchestration, full STEP B-rep semantics, measurement, backend export artifact history, durable cross-model assembly semantics, general CAD merge/boolean operations, normalized geometry APIs, or durable editable B-rep geometry as completed unless the code actually implements them. Persisted CAD Agent chat messages and OpenAI-compatible advisory replies are implemented, but they are not prompt-to-geometry generation or tool-using CAD automation. Persisted per-model transform operations, browser-worker STEP operation replay, a constrained per-model box-union feature, direct per-model STEP downloads, selected multi-model STEP compound downloads, and lightweight STEP product/component document-node display, positioning, and deletion are implemented, but they are not full B-rep feature-history, true geometric STEP decomposition, or durable assembly semantics.
