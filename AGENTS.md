...
The first LiteCAD-native feature DSL path is partially implemented: the CAD kernel worker can compile a minimal JSON DSL with numeric parameters and `box` features into OCCT preview mesh buffers or exported STEP text, the Assistant route accepts `litecad-feature-dsl` tool output, successful DSL artifacts can be saved as `.lcad.json` project models, saved DSL models preview through the project browser-kernel mesh pipeline, saved DSL models export to STEP through the project export UI, and saved DSL parameters can be edited with revision records. CAD document History integration, richer features, and full B-rep feature graph are still future work.
