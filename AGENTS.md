...
The first LiteCAD-native feature DSL path is partially implemented: the CAD kernel worker can compile a minimal JSON DSL with numeric parameters, rectangular `extrude`, `box`, `box_cut`, `cylinder`, and `cylinder_cut` features, optional non-zero cylinder axes, and bounded linear repeat patterns into OCCT preview mesh buffers or exported STEP text. `extrude` currently turns a rectangle sketch into a +Z solid, and `box_cut` subtracts rectangular slots, pockets, and edge notches from prior solids. Boolean and string parameters can be stored and edited as UI metadata, but geometry expressions may reference numeric parameters only. The Assistant route accepts `litecad-feature-dsl` tool output, successful DSL artifacts can be saved as `.lcad.json` project models, saved DSL models preview through the project browser-kernel mesh pipeline, saved DSL models export to STEP through the project export UI, and saved DSL parameters can be edited with revision records. CAD document History integration, full sketch constraints, non-rectangular profiles, fillets/chamfers, richer features, and full B-rep feature graph are still future work.
