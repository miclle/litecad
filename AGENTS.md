...
- Project-scoped CAD Agent chat with optional server-side OpenAI-compatible provider configuration, owner-scoped Assistant conversations, conversation-scoped persisted messages, provider context built from project/source metadata, and project-owned OpenSCAD-style parametric artifact draft storage APIs. It can advise about the project but does not mutate CAD documents or run geometry tools.
- Studio status endpoint for bootstrap/product-state messaging.

Do not document or implement AI-driven geometry mutation, AI tool orchestration, full STEP B-rep semantics, measurement, backend export artifact history, durable cross-model assembly semantics, general CAD merge/boolean operations, normalized geometry APIs, or durable editable B-rep geometry as completed unless the code actually implements them. Persisted CAD Agent conversations/messages, OpenAI-compatible advisory replies, and parametric artifact draft storage APIs are implemented, but they are not prompt-to-geometry generation or tool-using CAD automation. Database-backed Undo/Redo records LiteCAD's supported transform/box-union/node-delete commands, but it is not full B-rep feature-history, preserved source-application history, true geometric STEP decomposition, or durable assembly semantics.

AI-generated parameterized CAD is planned but not shipped until LiteCAD has structured tool calls that create artifacts, browser compilation, parameter editing, and save-as-project-model persistence. Do not copy CADAM code; use it only as product-flow reference.
