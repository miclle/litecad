...
- Project-scoped CAD Agent chat with optional server-side OpenAI-compatible provider configuration, owner-scoped Assistant conversations, conversation-scoped persisted messages, provider context built from project/source metadata, and a structured parametric-run API that validates strict `build_parametric_model` JSON output and creates project-owned pending OpenSCAD-style artifact drafts. The workbench can open generated drafts in an Inspector-side editor with parsed OpenSCAD-style parameter controls and compile-error state, successfully compiled artifacts can be saved as durable `.scad` project model sources, and saved `.scad` model parameters can be edited with separate revision records. It can advise about the project and create generated-source drafts, but it does not mutate CAD documents, compile generated source successfully without a runtime, preview generated mesh, or run geometry tools.
- Studio status endpoint for bootstrap/product-state messaging.

Do not document or implement AI-driven geometry mutation, CAD tool execution, full STEP B-rep semantics, measurement, backend export artifact history, durable cross-model assembly semantics, general CAD merge/boolean operations, normalized geometry APIs, or durable editable B-rep geometry as completed unless the code actually implements them. Persisted CAD Agent conversations/messages, OpenAI-compatible advisory replies, strict parametric-run tool JSON validation, parametric artifact draft creation, generated-source draft editing controls, `.scad` source model persistence for successful artifacts, and saved `.scad` parameter revision editing are implemented, but they are not successful browser mesh compilation, prompt-to-geometry mutation, or durable B-rep model generation. Database-backed Undo/Redo records LiteCAD's supported transform/box-union/node-delete commands, but it is not full B-rep feature-history, preserved source-application history, true geometric STEP decomposition, or durable assembly semantics.

The first LiteCAD-native feature DSL worker foundation is implemented: the CAD kernel worker can compile a minimal JSON DSL with numeric parameters and `box` features into OCCT preview mesh buffers or exported STEP text. This is a worker-level foundation only; it is not yet wired to the Assistant generation route, project model persistence, Inspector editor, project preview pipeline, CAD document History, or full B-rep feature graph.
...
- AI-generated parameterized CAD is not shipped end-to-end until LiteCAD has browser compilation, parameter editing, and save-as-project-model persistence for generated artifacts. Do not copy CADAM code; use it only as product-flow reference.
- Project data currently includes project metadata, static project-list thumbnail snapshots, uploaded STEP/STP/self-contained GLTF/GLB/STL/SCAD source-file records, lightweight source metadata with STEP product/component names, authenticated source download, editable LiteCAD document JSON for source model nodes plus STEP component child nodes and transform/delete-node/box-union operations, database-backed reversible command history with a persisted Undo/Redo head and discarded alternate paths, browser-kernel STEP workbench meshes derived with box-union replay plus scene-level persisted transforms, direct per-model STEP export downloads, selected multi-model STEP compound downloads, preview artifact metadata for backend-published GLTF/GLB/STL previews, derived STL OBJ preview artifacts, multi-source preview composition in the workbench, read-only model tree responses, generated geometry version snapshots, project-scoped CAD Agent conversations with conversation-scoped message history, strict parametric-run tool JSON validation, project-owned OpenSCAD-style parametric artifact draft records, draft parameter controls, save-as-`.scad` project model persistence for successful artifacts, saved `.scad` parameter revision editing, and a worker-level LiteCAD feature DSL compiler for numeric-parameter box geometry; it does not include durable kernel shape state, general CAD merge semantics, durable assembly records, backend export artifact persistence, Assistant-driven DSL persistence/UI, successful OpenSCAD browser mesh compilation of generated artifacts, or AI tool calls that mutate project geometry.
