...
Do not document or implement AI-driven geometry mutation, CAD tool execution, full STEP B-rep semantics, user-selectable or cross-revision topology measurement semantics, general mechanical or rotational mate solving beyond the shipped point-translation solver, reusable subassembly documents, general cross-model CAD merge/boolean operations, normalized geometry APIs, or durable editable B-rep geometry as completed unless the code actually implements them. Persisted CAD Agent conversations/messages, OpenAI-compatible advisory replies, strict parametric-run tool JSON validation, safe persisted Assistant failure messages for invalid provider tool output, parametric artifact draft creation, generated-source draft editing controls, LiteCAD DSL draft browser-kernel preview before save, `.lcad.json` source model persistence for successful LiteCAD DSL artifacts, saved `.lcad.json` browser-kernel mesh preview updates inside the existing viewer scene and STEP export, saved `.scad`/`.lcad.json` locally immediate parameter edits backed by immutable source/metadata model revisions with Inspector restore and reversible History entries, schema v4 nested organizational assembly groups with hierarchical suppression, deterministic acyclic `point-coincident-v1` mates with local anchors, world offset, residual, translation-only propagation and reversible History, preserved schema v3 unresolved mate records, backend-stored generated STEP export artifact history, separate preview-AABB and exact aggregate OCCT inspection records, deterministic same-input face/edge references, project-owned kernel-derived section-edge STEP association generations with explicit regeneration, and preview-layer edge/section/bounds inspection tools are implemented, but they are not full prompt-to-geometry mutation, successful OpenSCAD browser mesh compilation, durable B-rep model generation, a general mechanical constraint solver, automatically regenerating CAD document section features, section solids, or preserved source STEP assembly structure. Database-backed Undo/Redo records LiteCAD's supported occurrence/group/point-mate, solver-propagated transform, box-union, node-delete, and model revision transition commands inside the grouped assembly document, but it is not full B-rep feature history, preserved source-application history, true geometric STEP decomposition, or general mechanical solver semantics.
...
- Treat CAD tool execution, AI-driven geometry mutation, user-selectable or cross-revision topology measurement semantics, automatically regenerating CAD-document section features, general mechanical/rotational constraints beyond the shipped point-translation mate, reusable subassembly documents, preserved source STEP product structure, cross-model merge/boolean semantics, editable B-rep geometry, and full STEP B-rep semantics as roadmap work until code, tests, and UI flows exist. Schema v4 nested organizational groups, hierarchical suppression, shared model-revision occurrence bindings, independent name/order/group/suppression/placement authoring, and deterministic `point-coincident-v1` mates are implemented. The first occurrence drives the second occurrence's translation from two local anchors and a world offset through an acyclic single-inbound graph; driver edits propagate downstream atomically, History restores all changed placements, and migrated schema v3 unresolved records do not move geometry. Successful browser-kernel STEP exports are stored as project export artifact history for later download. Saved inspection records distinguish preview-derived AABB values from exact aggregate OCCT B-rep properties with deterministic face/edge references scoped to occurrence, immutable model revision, SHA-256 operation signature, kind, and index. Kernel section-edge STEP artifacts use immutable association generations with explicit stale/current/superseded state and user-triggered regeneration against the stored plane and current visible targets.
...
- Project data currently includes owner-scoped project/source records; editable CAD document schema v4 JSON with one durable assembly root, nested organizational groups, hierarchical suppression, immutable-revision-bound occurrences, deterministic `point-coincident-v1` translation mates, and migration-safe legacy unresolved mate records; source/component nodes and transform/delete-node/box-union operations; reversible database-backed History including solver-propagated placements; browser-kernel STEP and LiteCAD DSL preview/export; generated STEP artifact history; saved preview-AABB and exact aggregate OCCT inspection records; immutable section association generations; project-scoped CAD Agent conversations and strict parametric artifact generation; and versioned recursive `.lcad.json` source-graph editing. It does not include reusable durable kernel shape state, general mechanical or rotational constraint solving, reusable subassembly documents, preserved source STEP product structure, general cross-model CAD merge semantics, successful OpenSCAD browser mesh compilation, automatically regenerating section document features, cross-revision topology remapping, rich user-selectable B-rep metrology, or AI tool calls that mutate project geometry.
