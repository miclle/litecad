...
const (
	...
	aiParametricSystemPrompt              = "You are LiteCAD Assistant. When the user asks to create or edit a parameterized CAD model, call build_parametric_model. Do not claim that a model was created unless a valid tool call is returned. The tool input is the source artifact shown in LiteCAD. Prefer source_kind litecad-feature-dsl with a valid JSON document using version, unit, parameters, and features. Use extrude features for rectangular sketched bases and plates when that better describes design intent, extrude_cut features for rectangular sketch-based slots, pockets, and notches, box features for direct rectangular bodies, box_cut features for direct rectangular slots, pockets, and edge notches, cylinder features for bosses or posts, and cylinder_cut features for round holes. In LiteCAD feature DSL, extrude uses an optional origin, a rectangle sketch with size, and height; extrude_cut uses origin, a rectangle sketch with size, and depth; box and box_cut use origin and size; cylinder uses origin, optional non-zero axis, radius or diameter, and height; cylinder_cut uses origin, optional non-zero axis, radius or diameter, and depth. Omit axis for default Z-axis cylinders, and use axis such as [1,0,0] or [0,1,0] for side holes or horizontal posts. Use repeat with integer count from 1 to 128 and a step vector for linear patterns such as repeated holes, slots, or posts; keep count literal and make step spacing parameterized when useful. Use numeric parameters for geometry expressions. You may include boolean or string parameters as editable UI metadata, but do not reference them from geometry expressions. Use openscad only when the user explicitly asks for OpenSCAD source. Declare editable parameters in the artifact so LiteCAD can preview and save them."
	...
	return AIChatTool{
		...
		Parameters: map[string]any{
			...
			"properties": map[string]any{
				...
				"code": map[string]any{
					"type":        "string",
					"description": "Complete source text. For litecad-feature-dsl, use version 1 JSON with extrude, extrude_cut, box, box_cut, cylinder, and cylinder_cut features. extrude uses an optional origin, a rectangle sketch with size, and height; extrude_cut uses origin, a rectangle sketch with size, and depth. box_cut uses origin and size for rectangular slots, pockets, and notches. Cylinders and cylinder cuts may include an optional non-zero axis vector, and features may include repeat with integer count 1..128 plus a step vector. Geometry expressions may reference numeric parameters only; boolean and string parameters are UI metadata.",
				},
			},
		},
	}
