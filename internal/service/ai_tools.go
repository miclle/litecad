...
func buildAIParametricSystemPrompt() string {
	...
	) + aiParametricHollowRevolvePrompt
}
	...
	tool := AIChatTool{
		...
		Parameters: map[string]any{
			...
			"properties": map[string]any{
				...
				"code": map[string]any{
					"type":        "string",
					"description": "Complete source text. For litecad-feature-dsl, use version 1 JSON and the backend capability registry: sketch, box, box_cut, extrude, extrude_cut, cylinder, cylinder_cut, sphere, ellipsoid, ellipse_extrude, tapered_extrude, revolve, sweep, loft, fillet, chamfer, and boolean. sketch defines reusable rectangle/circle/ellipse profiles; extrude and tapered_extrude sketch references must use XY profiles, and tapered_extrude requires positive height and positive top_scale; revolve/sweep/loft may use XY/XZ/YZ profiles. revolve references a sketch plus axis_origin/axis/angle_degrees; sweep uses a sketch and path points; loft uses two or more sections; boolean uses operation union/subtract/intersect and inline solid operands; fillet applies one radius and chamfer applies one symmetric distance to all eligible edges of the accumulated shape; invalid modifier geometry must fail. Features may include repeat with integer count 1..128 plus a step vector, and generated solid features may include transform with optional translate [x,y,z], rotate {axis, angle_degrees, origin}, and positive scale [sx,sy,sz]. Geometry expressions may reference numeric parameters directly or use structured numeric expression objects with op add/sub/mul/div and two args; boolean and string parameters are UI metadata.",
				},
			},
		},
	}
