...
func TestAIParametricToolSchemaDescribesHollowRectangularRevolve(t *testing.T) {
	tool := buildParametricModelAITool()
	properties := tool.Parameters["properties"].(map[string]any)
	codeSchema := properties["code"].(map[string]any)
	description := codeSchema["description"].(string)
	for _, want := range []string{"hollow full-turn revolve", "XZ rectangle", "must not cross the revolve axis", "profile plane must contain the revolve axis"} {
		if !strings.Contains(description, want) {
			t.Fatalf("tool code schema should mention %q, got %s", want, description)
		}
	}
}
