...
func TestAIParametricRunCreatesPendingLiteCADFeatureDSLArtifact(t *testing.T) {
  ...
  "input": {
	...
	for _, want := range []string{"litecad-feature-dsl", "box", "extrude", "extrude_cut", "circle", "cylinder", "cylinder_cut", "holes", "non-zero axis", "repeat", "expression", "add", "boolean", "string", "geometry"} {
