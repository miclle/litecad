...
func TestGetProjectCADDocumentCreatesPersistedIdentityDocument(t *testing.T) {
	...
	if document.SchemaVersion != 4 || document.Unit != "millimetre" {
...
func TestGetProjectCADDocumentPreservesExistingV2OccurrenceIdentityAndDuplicates(t *testing.T) {
	...
	if document.SchemaVersion != 4 || document.Revision != 8 || len(document.Assembly.Occurrences) != 2 {
		t.Fatalf("synced document schema/revision/occurrences = %d/%d/%d, want 4/8/2", document.SchemaVersion, document.Revision, len(document.Assembly.Occurrences))
	}
...
func TestGetProjectCADDocumentUpgradesV1TransformToAssemblyOccurrence(t *testing.T) {
	...
	if upgraded.SchemaVersion != 4 || upgraded.Revision != 5 {
		t.Fatalf("upgraded document schema/revision = %d/%d, want 4/5", upgraded.SchemaVersion, upgraded.Revision)
	}
