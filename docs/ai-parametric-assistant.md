...
The LiteCAD-native feature DSL path is now connected to Assistant tool validation, generated artifact persistence, save-as-project-model persistence, saved parameter revision editing, and project preview. A successful `litecad-feature-dsl` artifact is stored as a durable `.lcad.json` project model with `format = "lcad"` and metadata for schema, unit, parameter values, and feature count. The Inspector can read the DSL parameter defaults and edit saved parameter values. Saved `.lcad.json` models are routed through the existing `feature-dsl-preview` worker path for browser-kernel mesh preview. Export UI for saved DSL models through `feature-dsl-export` remains future work.
