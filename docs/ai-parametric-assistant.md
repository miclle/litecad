...
Because the OpenSCAD runtime remains blocked by license or distribution constraints, LiteCAD feature DSL JSON is the preferred generated-source kind. The current DSL foundation supports numeric, boolean, and string parameter metadata at the artifact/model layer, plus numeric-parameter rectangular `extrude`, `box`, `box_cut`, `cylinder`, and `cylinder_cut` features in the OCCT worker compiler. Boolean and string parameters are editable UI metadata and are preserved through save/update revisions, but geometry expressions may reference numeric parameters only. `extrude` currently supports a rectangle sketch extruded along +Z into a solid; `box_cut` subtracts rectangular slots, pockets, and edge notches from prior solid features. Cylinders and cylinder cuts default to the Z axis and can specify an optional non-zero `axis` vector for side holes and horizontal posts. Any supported feature can include a bounded linear `repeat` pattern with literal integer `count` from 1 to 128 and a parameterizable `step` vector.
