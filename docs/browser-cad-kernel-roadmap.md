...
The first browser-kernel proof of concept has proven import, tessellation, constrained per-model edit replay, direct per-occurrence STEP export, selected multi-occurrence compound STEP export, backend-stored generated STEP export artifact history, preview-layer inspection aids for edge display and center-plane visual sectioning, visible-model bounds plus diagonal measurement, exact aggregate OCCT B-rep analysis with deterministic immutable-input face/edge references, project-saved measurement/section inspection records, and project-owned OCCT section-edge STEP artifacts with explicit association generations and regeneration. LiteCAD now persists immutable source/metadata model revisions, one CAD document schema v3 assembly whose occurrences own name, order, parent group, suppression, revision binding, and placement, nested organizational groups with hierarchical suppression, validated unresolved mate records, and reversible command History for occurrence/group/constraint-record authoring, supported geometry edits, parameter/revision restores, and versioned recursive LiteCAD Feature DSL source-graph replacements with globally stable node IDs and path-aware transitions. A durable database schema for reusable serialized kernel shape state, cross-revision B-rep topology naming/remapping, rich parametric B-rep feature semantics, solver-backed assembly constraints, reusable subassembly documents, and automatically regenerating CAD-document section features is still future design work.
...
- No reusable durable kernel shape serialization, rich parametric B-rep feature model, cross-revision topology remapping, user-selectable point/edge/face metrology, solver-backed assembly constraints, reusable subassembly documents, automatically regenerating CAD-document section features, preserved source STEP product structure, or cross-model CAD merge semantics exist yet.
