...
LiteCAD now stores AI-generated OpenSCAD-style and LiteCAD feature DSL source artifacts separately from uploaded STEP/GLTF/GLB/STL assets. Generated artifacts can expose parameters in the Inspector, pending LiteCAD DSL drafts preview through the browser `feature-dsl-preview` worker before save, successful OpenSCAD artifacts can be saved as durable `.scad` project model sources, successful LiteCAD DSL artifacts can be saved as durable `.lcad.json` project model sources, saved `.lcad.json` models preview through the same worker path, and saved `.scad`/`.lcad.json` parameter changes are recorded as separate revision records. These generated-source records are source assets, not browser-kernel B-rep shape state.
