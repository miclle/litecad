...
- CAD document schema v4 stores one project assembly, nested organizational groups, deterministic point mates, migration-safe legacy unresolved records, and one or more stable occurrences per active root model. The workbench renders the assembly root, group/occurrence authoring controls, and point-mate authoring/status controls; occurrences reference immutable model revisions and independently own name, order, parent group, suppression, and placement. Direct or ancestor group suppression excludes an occurrence from preview and export. Occurrence placement remains the only persisted top-level transform owner, solver updates are server-authoritative, and owner-scoped revision-source downloads ensure export reads pinned source bytes even if another browser changes the model's current revision.
...
- How should rotational and topology-referenced constraints, reusable subassembly documents, colors, names, units, and source STEP product structure extend the current schema v4 organizational group and point-mate model?
...
- The project workbench stores and reloads a LiteCAD editable document schema v4 with one durable assembly, nested organizational groups with hierarchical suppression, deterministic point-translation mates plus migration-safe unresolved legacy records, one or more immutable-revision-bound model occurrences that own name, order, parent group, suppression, and placement, uploaded source model node compatibility projections, independently selectable STEP component child nodes, node-delete, and constrained box-union operations.
...
- No reusable durable kernel shape serialization, rich parametric B-rep feature model, cross-revision topology remapping, user-selectable point/edge/face metrology, general mechanical/rotational assembly solver, reusable subassembly documents, automatically regenerating CAD-document section features, preserved source STEP product structure, or cross-model CAD merge semantics exist yet.
