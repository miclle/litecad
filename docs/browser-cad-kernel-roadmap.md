...
The current implementation covers the first browser-kernel loop for STEP models: STEP workbench preview replays constrained box-union geometry during worker tessellation and creates one Three.js scene instance per effectively unsuppressed durable assembly occurrence. CAD document schema v4 supports nested organizational groups, deterministic `point-coincident-v1` translation mates, and immutable project-local reusable-subassembly snapshots expanded into revision-pinned member occurrences; suppression on any ancestor group excludes its descendant occurrences from preview and export. One immutable model revision may have multiple independently named, ordered, grouped, suppressed, and placed ordinary occurrences or pinned reusable-instance members. STEP export derives occurrence order, immutable revision binding, name, and solver-resolved or instance-pinned placement from those occurrences, and the workbench can download selected occurrences either as separate files or as one browser-kernel compound STEP file. GLTF/GLB/STL preview artifacts remain viewer outputs rather than editable CAD documents.
...
- `shapeRef`: deterministic reference to a kernel subshape within one immutable occurrence/revision/operation-signature scope; it is not a cross-revision topology identity.
...
The first browser-kernel proof of concept has proven import, tessellation, constrained per-model edit replay, direct per-occurrence STEP export, selected multi-occurrence compound STEP export, backend-stored generated STEP export artifact history, preview-layer inspection aids for edge display and center-plane visual sectioning, visible-model bounds plus diagonal measurement, exact aggregate OCCT B-rep analysis with deterministic immutable-input face/edge references, project-saved measurement/section inspection records, and project-owned OCCT section-edge STEP artifacts with explicit association generations and regeneration. LiteCAD now persists immutable source/metadata model revisions, one CAD document schema v4 assembly whose occurrences own name, order, parent group, suppression, revision binding, and placement, nested organizational groups with hierarchical suppression, deterministic translation-only point mates plus migration-safe legacy unresolved records, immutable revision-1 reusable-subassembly definitions with expanded pinned instances, and reversible command History for occurrence/group/constraint/reusable-definition/reusable-instance authoring, solver-propagated placements, supported geometry edits, parameter/revision restores, and versioned recursive LiteCAD Feature DSL source-graph replacements with globally stable node IDs and path-aware transitions. Durable reusable serialized kernel shape state, cross-revision B-rep topology naming/remapping, rich parametric B-rep feature semantics, general mechanical/rotational constraint solving, live/nested/cross-project reusable subassembly documents, and automatically regenerating CAD-document section features are still future design work.

## Generated Parametric Source Status

LiteCAD now stores AI-generated OpenSCAD-style and LiteCAD feature DSL source artifacts separately from uploaded STEP/GLTF/GLB/STL assets. Every saved project model receives an immutable revision 1 snapshot of source bytes and metadata; legacy rows are backfilled lazily and idempotently. Parameter edits and Assistant-applied complete validated saved-`.lcad.json` source-graph replacements create child revisions, CAD document assembly occurrences bind the active revision ID, and the Inspector can list and restore revisions through the same expected-revision and Undo/Redo envelope. Graph updates preserve the parameter schema/value boundary, require globally unique already-trimmed IDs across top-level features and recursive boolean operands, compile in the browser before Apply, and record graph version plus deterministic added/updated/moved/removed transitions with JSON-Pointer-safe paths and before/after sibling indexes. The ordinary Inspector stays parameter-first and exposes immutable revision restore without source-level graph or complete-source editing; selected-model Assistant revisions retain the validated graph-replacement and History contract. Root model nodes expose the revision binding only as a compatibility projection. The model row retains current source/metadata as a compatibility cache while `ProjectModelRevision` is the durable version identity. OpenSCAD drafts remain parameter-editable source records but do not compile to browser meshes or follow the normal save-to-canvas path. These source revisions are durable source truth, not serialized OCCT shapes, cross-revision B-rep topology identities, or browser-kernel B-rep feature state.
...
The eighth Phase 3 increment is complete on 2026-07-15: saved LiteCAD Feature DSL source revisions now form a versioned recursive source graph. Every top-level feature and recursive boolean operand shares one globally unique stable ID namespace, History records deterministic path/index-aware added/updated/moved/removed transitions, and the Inspector edits one selected node through a graph rail while retaining advanced complete-source structural editing. Go and browser validation share the same identity rules, browser-kernel compilation gates Apply, and Undo/Redo restores exact immutable source revisions across reloads. This is not serialized OCCT state, cross-revision B-rep topology naming, sketch constraints, or imported source feature history.

The ninth Phase 3 increment is complete on 2026-07-15: the browser worker rebuilds revision-pinned occurrence shapes and reports exact OCCT aggregate volume, surface area, edge length, center of mass, and solid/face/edge counts. Face and edge references are deterministic only inside `(occurrence_id, model_revision_id, sha256_operations_signature, kind, one_based_index)`, and the server reconstructs and validates that provenance before persisting an `occt-brep-properties` inspection record. Preview-visible AABB measurement remains a separately labeled viewer aid. Section artifacts now belong to immutable associations with monotonic generations; changing the document revision or visible occurrence/revision targets marks an older latest generation stale, and an explicit Regenerate action appends a new generation for the original plane while retaining superseded results. This is not cross-revision topology remapping, user-selectable point/edge/face metrology, automatic section regeneration, a CAD-document section feature, or a section solid.

The tenth Phase 3 increment is complete on 2026-07-15: CAD document schema v4 adds deterministic `point-coincident-v1` mates. The first occurrence drives the second occurrence's translation from two occurrence-local anchors and one world-space offset while preserving the driven 3 x 3 transform. Solver constraints form an acyclic graph with at most one inbound driver per occurrence; moving a driver resolves all downstream placements atomically, and History restores every affected occurrence through Undo/Redo. Owner-scoped APIs create and delete mates. The default workbench keeps existing links in collapsed advanced management, allows removal, and keeps driven Inspector inputs and canvas transform controls read-only; it no longer exposes raw coordinate authoring or residuals to general users. Schema v3 unresolved records upgrade without movement. This is not rotational, plane/axis/concentric, tolerance, over-constraint, or topology-reference solving, and it does not add reusable subassembly documents.

The eleventh Phase 3 increment is complete on 2026-07-15: schema v4 persists immutable project-local revision-1 subassembly definitions captured from the direct ordinary occurrences of one leaf group. Members pin node/model/revision identity, name, suppression, and transforms normalized to the first member's translation. Instantiation creates a tagged group plus expanded read-only occurrences at an explicit XYZ translation, so preview, inspection, hierarchical suppression, and separate/compound export continue through the existing occurrence pipeline. Definition capture and instance creation are expected-revision mutations with one reversible History command each; whole-instance suppression is also reversible. Current model revisions may advance without rewriting pinned members. This does not add definition evolution/deletion, editable members, nested definitions or instances, live propagation, cross-project libraries, source STEP hierarchy preservation, or nested STEP serialization.
...
- Every project model has immutable source/metadata revisions; saved `.scad` and `.lcad.json` parameter edits create child revisions, revision list/detail/restore APIs are owner-scoped, and both edits and restores use revision-ID History transitions. Saved `.lcad.json` graph replacements add durable graph-versioned recursive source-node transitions, but OCCT shapes and B-rep topology identities remain derived runtime state.
- CAD document schema v4 stores one project assembly, nested organizational groups, deterministic point mates, migration-safe legacy unresolved records, project-local immutable reusable-subassembly definitions/instances, and one or more stable occurrences per active root model. The workbench renders the assembly root, group/occurrence authoring controls, collapsed advanced management for existing position links, and reusable capture/instantiate controls; raw point-mate coordinates remain an API-level capability until a novice-facing canvas or automatically derived workflow exists. Occurrences reference immutable model revisions and independently own name, order, parent group, suppression, and placement. Reusable instances expand pinned members into tagged groups and allow only whole-group suppression. Direct or ancestor group suppression excludes an occurrence from preview and export. Occurrence placement remains the only persisted top-level transform owner, solver updates are server-authoritative, reusable-member placement is immutable, and owner-scoped revision-source downloads ensure export reads pinned source bytes even if another browser changes the model's current revision.
...
- This phase still does not claim durable kernel shape serialization, preserved source-application history, rich parametric B-rep feature semantics, assembly constraints beyond deterministic point-translation mates, reusable-definition evolution/deletion, live/nested/cross-project subassembly documents, general cross-model boolean/merge workflows, or preserved source STEP product structure.
...
Phase 6 acceptance status: complete on 2026-07-14 and extended on 2026-07-15. `section-geometry` rebuilds STEP or LiteCAD Feature DSL occurrence shapes from pinned revisions, replays supported operations and placement, applies an OCCT B-rep section plane, and returns either STEP edge geometry or a typed empty result. Owner-scoped APIs persist the document revision, plane, units, source revision IDs, occurrence IDs, edge count, byte size, status, and ready artifact bytes. Every new section starts an immutable association at generation 1; explicit regeneration row-locks that association, verifies the expected current generation and original plane, then appends the next generation without deleting superseded results. The workbench lists artifacts across reloads, marks current/stale/superseded/legacy state, and supports explicit regeneration, plane restore, STEP download, and deletion. Visible-bounds measurement also reports an explicitly `preview-visible-aabb`-derived diagonal, while a separate worker path reports exact aggregate OCCT B-rep properties.

Boundary:

- A ready artifact generation is an immutable STEP serialization of OCCT intersection edges. Its stable association and monotonic generation lineage supports explicit user-triggered regeneration, but it is not a section solid, reusable serialized document shape, automatic background regeneration, or a CAD-document feature with Undo/Redo.
- An empty intersection is stored as a typed empty result with zero edges and no STEP bytes.
- Visible X/Y/Z dimensions and diagonal are derived from the combined visible preview AABB. Exact aggregate volume, surface area, edge length, center of mass, and topology counts use OCCT and are stored separately.
- Exact face/edge references are stable only for the same occurrence, immutable model revision, SHA-256 operation signature, topology kind, and index. They do not remap topology across revisions or expose user-selectable point/edge/face semantics.
- Source revision, occurrence, document revision, and plane provenance drive explicit stale-state detection; regeneration preserves the original plane and appends rather than mutating a prior result.

Tests and verification:

- Go service and handler coverage validates ready/empty artifacts, ownership, metadata, list/download/delete, payload limits, association generation conflicts, immutable planes, and race-safe latest-generation updates.
- Real OCCT worker tests section a 60 x 24 x 8 box into four edges, verify an out-of-range plane returns empty, and measure a 10 x 20 x 30 box as volume 6000, area 2200, edge length 240, center `(5, 10, 15)`, 1 solid, 6 faces, and 12 edges with repeatable references.
- Deterministic Playwright covers exact analysis plus generate, model-revision stale state, generation-2 regeneration, persist across reload, restore, download, and delete through the workbench.
- In-app browser verification against the real local backend measured a 60 x 24 x 8 LiteCAD box as volume 11,520, area 4,224, and edge length 368; changing width to 70 marked section generation 1 stale, explicit regeneration created current generation 2 and retained generation 1 as superseded, reload preserved the lineage, and the browser found no console warnings or errors.
...
- How should rotational and topology-referenced constraints, reusable-definition revision evolution/deletion, nested or live-linked definitions, cross-project libraries, colors, names, units, and source STEP product structure extend the current schema v4 organizational group, point-mate, and immutable snapshot model?
...
- The project workbench can overlay mesh edges, visually clip the current preview through a center plane, display visible-model AABB dimensions plus diagonal, run and save exact aggregate OCCT B-rep analysis with deterministic immutable-input face/edge references, save viewer-derived measurement/section definitions, and persist exact OCCT section-edge STEP artifacts with source-revision/occurrence provenance. The Analysis popover derives a current view from the active CAD document revision and visible occurrence IDs, keeps only the latest matching result per measurement category in that view, preserves the latest stale section association as a regeneration action, and exposes every other persisted result through collapsed history. Section artifacts use immutable association generations with explicit current/stale/superseded state and manual regeneration; they are not automatic CAD-document features or section solids.
- The project workbench stores and reloads a LiteCAD editable document schema v4 with one durable assembly, nested organizational groups with hierarchical suppression, deterministic point-translation mates plus migration-safe unresolved legacy records, immutable project-local revision-1 reusable-subassembly definitions with tagged expanded pinned instances, one or more immutable-revision-bound model occurrences that own name, order, parent group, suppression, and placement, uploaded source model node compatibility projections, independently selectable STEP component child nodes, node-delete, and constrained box-union operations.
- Database-backed History stores reversible occurrence, group, point-mate, reusable-definition, reusable-instance, solver-propagated transform, node-delete, box-union, saved parameter-revision, and versioned Feature DSL graph commands, plus the active Undo/Redo head and discarded alternate paths; every CAD document mutation uses an expected document revision.
...
- No reusable durable kernel shape serialization, rich parametric B-rep feature model, cross-revision topology remapping, user-selectable point/edge/face metrology, general mechanical/rotational assembly solver, live/nested/cross-project reusable subassembly documents, reusable-definition evolution/deletion, automatically regenerating CAD-document section features, preserved source STEP product structure, or cross-model CAD merge semantics exist yet.
