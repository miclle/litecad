...
LiteCAD now stores AI-generated OpenSCAD-style and LiteCAD feature DSL source artifacts separately from uploaded STEP/GLTF/GLB/STL assets. Generated artifacts can expose parameters in the Inspector, pending LiteCAD DSL drafts preview through the browser `feature-dsl-preview` worker before save, successful LiteCAD DSL artifacts can be saved as durable `.lcad.json` project model sources, saved `.lcad.json` models preview through the same worker path, and saved `.lcad.json` parameter changes are recorded as separate revision records that survive reload. The backend storage model can represent `.scad` source models and parameter revisions, but OpenSCAD drafts do not compile to browser meshes or follow the normal save-to-canvas path until a compatible OpenSCAD runtime is selected. These generated-source records are source assets, not browser-kernel B-rep shape state.
...
The first feature DSL worker foundation is complete on 2026-07-11 and the first feature graph / AST pass is complete on 2026-07-13. The CAD kernel worker now accepts `feature-dsl-preview` and `feature-dsl-export` requests for a minimal JSON document with numeric parameters, structured numeric expressions, and supported features including `box`, `box_cut`, `cylinder`, `sphere`, faceted `ellipsoid`, `ellipse_extrude`, `cylinder_cut`, rectangular/circular/elliptical `extrude`, rectangular/circular/elliptical `extrude_cut`, reusable `sketch` definition nodes, `revolve`, straight-path `sweep`, multi-section `loft`, recursive boolean-tree `union` / `subtract` / `intersect`, `fillet`, conservative `chamfer`, optional non-zero cylinder axes, optional positive/negative/symmetric sketch extrusion direction, bounded repeat patterns, and feature-local `translate` / `rotate` / positive non-uniform `scale` transforms. Browser verification through real Vite/Chromium worker runs compiled a parameterized 96 x 42 x 6 box into a mesh with 24 vertices, 12 triangles, normals present, and 15403 bytes of exported STEP text, then compiled a box plate with a parameterized cylinder through-hole into a mesh with 130 vertices, 120 triangles, normals present, and 19014 bytes of exported STEP text. The Assistant route now accepts `litecad-feature-dsl` tool output, pending DSL drafts compile through `feature-dsl-preview` before Save is enabled, successful DSL artifacts can persist as `.lcad.json` project models, the project preview pipeline routes saved `.lcad.json` models through `feature-dsl-preview`, the project export UI routes saved `.lcad.json` models through `feature-dsl-export` for separate or merged STEP downloads, and the Inspector can edit saved `.scad` and `.lcad.json` parameter values with immediate local preview updates, debounce-persisted revisions, reload persistence, and reversible History entries. These generated artifacts remain source-model records, not durable kernel shape state or AI-authored CAD document mutations. The remaining browser-kernel gaps are full sketch constraints, freeform profiles, arbitrary sweep paths, robust true chamfer edge-face selection, mirrored transforms, durable kernel shape state, CAD document History integration for generated feature graph nodes, and backend export artifact history.
...
- Saved `.scad` and `.lcad.json` parameter edits are recorded as model revisions and reversible History entries, but they remain source-model parameter changes rather than durable kernel feature graph mutations.
