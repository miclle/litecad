...
- Export currently covers selected STEP source models plus their replayable LiteCAD operations, and saved `.lcad.json` Feature DSL source models through `feature-dsl-export`. The merged output is an OCCT compound STEP download, not a durable editable assembly record, backend export artifact, preserved source CAD feature history, or serialized durable kernel shape state.
