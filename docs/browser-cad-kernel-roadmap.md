...
- This phase still does not claim durable kernel shape serialization, preserved source-application history, rich parametric B-rep feature semantics, nested assemblies, mates/constraints, general cross-model boolean/merge workflows, preserved source STEP product structure, or backend STEP export artifact storage.
...
- Export currently covers selected STEP source occurrences plus their replayable LiteCAD geometry operations and occurrence placement, and saved `.lcad.json` Feature DSL source occurrences through `feature-dsl-export` followed by placement replay. The merged output is an OCCT compound derived from the durable flat assembly, not preserved nested STEP assembly structure, a backend export artifact, preserved source CAD feature history, or serialized durable kernel shape state.
...
- No durable kernel shape serialization, rich parametric B-rep feature model, nested assembly/mate/constraint semantics, durable CAD measurement/section records, backend export artifact history, preserved source STEP product structure, or cross-model CAD merge semantics exist yet.
