...
- Continue shrinking the project workbench hotspots after moving serialized CAD command coordination, revision conflicts, transform autosave, STEP export feedback, History controls, and the Assistant panel out of `website/src/views/project/index.tsx`. Next extract the model tree and inspector surfaces, then split scene/resource/selection lifecycles in `website/src/views/project/model-preview.tsx` while keeping renderer lifecycle, view events, and resource disposal aligned with the project rules.
