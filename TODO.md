...
- Continue assembly semantics beyond the completed CAD document schema v3 organizational tree, where nested groups own hierarchical suppression, occurrences may reference one immutable model revision and independently own name, order, group, suppression, placement, deletion, History, and separate or compound STEP export semantics.
  - Add solver-backed mate/constraint definitions, geometric references, solve status, and placement updates beyond the current validated `unresolved` mate records, which intentionally do not move geometry.
