...
- Define assembly semantics for multi-source projects beyond completed per-model transform persistence, per-model box-union edits, and per-model direct STEP export, including selection, explicit cross-model merge/boolean behavior, and combined assembly export behavior. Current multi-model support is browser preview composition plus persisted placement; it does not combine multiple STEP files into one editable CAD part.
