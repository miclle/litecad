...
- Continue the AI Parametric Assistant described in [docs/ai-parametric-assistant.md](docs/ai-parametric-assistant.md) and [docs/superpowers/plans/2026-07-11-ai-parametric-assistant.md](docs/superpowers/plans/2026-07-11-ai-parametric-assistant.md): browser compile/preview for generated OpenSCAD-style artifacts and later LiteCAD-native OCCT feature DSL support. Multi-session Assistant conversations, strict `build_parametric_model` JSON validation, pending artifact draft creation, an Inspector-side draft editor with parsed parameter controls, save-as-`.scad` project model persistence for successful artifacts, and saved `.scad` parameter revision editing are implemented. The compatible browser compile runtime is still required before generated source can preview as mesh in normal use.
  - Add a license-compatible OpenSCAD browser runtime or intentionally switch generated models to a LiteCAD-native OCCT feature DSL for durable STEP/B-rep editing beyond OpenSCAD-style source assets. The first feature DSL worker foundation can already compile numeric-parameter box features to OCCT mesh/STEP, but it is not yet connected to Assistant generation, project persistence, or the Inspector UI.
  - Add Assistant cost controls, provider-specific structured tool-call support, and richer Assistant run failure states before exposing longer generation workflows.
