...
- Continue assembly semantics beyond the completed CAD document schema v2 flat assembly, where each model occurrence pins an immutable model revision, owns placement, participates in document History, and drives separate or compound STEP export.
  - Define nested subassemblies, occurrence reordering/editing commands, mates, constraints, and suppression semantics.
...
- Expand export beyond the current direct browser download path.
  ...
  - Keep current selected multi-model STEP output described as an OCCT compound derived from durable flat occurrences, not preserved nested STEP assembly structure.
