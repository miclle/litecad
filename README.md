...
- A project workbench route with a CAD-style Three.js viewer shell, source-file list, upload control, browser-kernel STEP preview meshes with persisted transform operation replay, backend-published GLTF/GLB/STL preview artifacts, persisted per-model transform controls, and multi-model frontend viewer composition.
...
AI model orchestration, full STEP geometry/B-rep semantics, CAD merge/boolean operations, normalized editable B-rep geometry, measurement tools, export, and rich design-history persistence are product direction, not completed capabilities yet.

The target CAD architecture is an embedded browser CAD kernel based on OCCT/OpenCascade.js or an equivalent WebAssembly geometry kernel. The long-term loop is STEP import, browser-side B-rep editing, derived Three.js preview meshes, and STEP export without requiring users to install FreeCAD or another desktop CAD application. The current project workbench previews STEP sources through the browser kernel worker and persists per-model transform edits in a LiteCAD document; STEP preview replay applies those transform operations inside the worker before tessellation. True feature/B-rep edit operations and STEP export UI are still roadmap work tracked in [Browser CAD Kernel Roadmap](docs/browser-cad-kernel-roadmap.md) and [TODO.md](TODO.md).
