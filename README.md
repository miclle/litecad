...
- Database-backed operation History with owner-scoped Undo/Redo for occurrence creation/update/reorder/deletion, assembly group and unresolved mate-record changes, transforms, box unions, node deletion, saved parameter revisions, and saved LiteCAD Feature DSL graph revisions, including persisted redo state across reloads and devices.
- A durable CAD document schema v3 assembly: one model revision can have multiple independently named, ordered, grouped, suppressed, placed, and deleted occurrences. Groups may be nested, ancestor suppression propagates to preview and export, and each occurrence pins an immutable model revision inside the same document revision and History concurrency envelope. Owner-scoped APIs can also store validated `mate` records between two occurrences with status `unresolved`; these records do not move geometry or imply a constraint solver.
...
Near-term product work is focused on turning the current preview/edit/export loop into a richer CAD workflow: durable kernel shape state, broader feature operations, cross-revision topology naming and user-selectable point/edge/face measurement semantics beyond current exact aggregate analysis, automatic or CAD-document-integrated section regeneration beyond the current explicit association lineage, solver-backed assembly constraints and reusable subassemblies, stronger CAD Agent tool boundaries, and project management polish.

Active follow-up items are tracked in [TODO.md](TODO.md). The browser-kernel architecture plan and phase history live in [docs/browser-cad-kernel-roadmap.md](docs/browser-cad-kernel-roadmap.md), and the exact schema v3 grouping/constraint-record boundary lives in [docs/nested-assembly-semantics.md](docs/nested-assembly-semantics.md). The current cross-machine handoff is [docs/current-work-handoff.md](docs/current-work-handoff.md).
