...
- A project workbench with a Three.js viewer, an explicit project assembly root with flat model occurrences, document inspector, ViewCube/orientation controls, model visibility toggles, multi-model preview composition, and locally restored left-panel/Assistant visibility and widths.
...
- Database-backed operation History with owner-scoped Undo/Redo for transforms, box unions, node deletion, and immutable model revision transitions, including persisted redo state across reloads and devices.
- A durable CAD document schema v2 flat assembly: each model occurrence pins an immutable model revision and owns its placement inside the same document revision and History concurrency envelope; export source downloads resolve that exact revision rather than the model's mutable current-version cache.
- Direct per-model STEP downloads and selected multi-model STEP compound downloads derived from the durable occurrence order, revision bindings, and placements.
