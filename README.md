...
- A project workbench route with a CAD-style Three.js viewer shell, source-file list, upload control, FreeCAD-backed OBJ preview meshes for STEP files, and a frontend viewer that only consumes backend-provided preview artifacts.
...
The backend converts STEP sources through a format-neutral preview artifact boundary. The current FreeCAD converter emits OBJ because FreeCAD's headless `Mesh.export` path in the local toolchain does not export GLB directly, but the service layer already accepts GLB preview bytes from a converter. GLTF, GLB, and STL uploads are stored as source records, but they do not currently become viewer preview artifacts until the backend normalizes them into a Three.js-friendly artifact. The preview artifact metadata endpoint exposes the artifact format, content type, byte size, and mesh counts separately from the binary payload so clients can choose the correct loader before downloading preview bytes. The read-only geometry document endpoint exposes the current project model tree, preview artifacts, and generated geometry version records without claiming editable CAD semantics.
