...
- A project workbench with a Three.js viewer, an explicit project assembly root with flat model occurrences, compact occurrence duplicate/rename/reorder/suppress/delete controls, occurrence placement editing, document inspector, ViewCube/orientation controls, model visibility toggles, multi-model preview composition, and locally restored left-panel/Assistant visibility and widths.
...
- Database-backed operation History with owner-scoped Undo/Redo for occurrence creation/update/reorder/deletion, transforms, box unions, node deletion, saved parameter revisions, and saved LiteCAD Feature DSL graph revisions, including persisted redo state across reloads and devices.
- A durable CAD document schema v2 flat assembly: one model revision can have multiple independently named, ordered, suppressed, placed, and deleted occurrences. Each occurrence pins an immutable model revision inside the same document revision and History concurrency envelope; export source downloads resolve that exact revision rather than the model's mutable current-version cache.
...
LiteCAD is not a full parametric CAD system yet. Current edits are limited to flat assembly occurrence duplication, naming, ordering, suppression, placement and deletion, model/source node deletion with STEP component child deletion, a constrained STEP box-union operation, source-model revision transitions, and complete validated source-graph replacement for saved `.lcad.json` models. Every project model has an immutable source/metadata revision chain; parameter edits, top-level Feature DSL graph edits, and Inspector restores move the current revision through reversible History with stale-write protection. Graph History reports stable top-level node IDs as added, updated, or removed, but it is not preserved source-application history, nested boolean-operand editing, serialized OCCT shape state, or a general parametric B-rep feature graph. Projects do not yet store durable kernel shape state, editable B-rep geometry, nested assemblies, mates or constraints, cross-model boolean features, or durable B-rep section bodies. Stored export artifacts are generated STEP files for download history, and saved measurement/section records are viewer-derived inspection snapshots/definitions tied to document revisions, not durable kernel shape state or preserved source CAD feature history. The flat assembly does not preserve arbitrary source STEP product structures. The current edge, section, and measurement controls remain viewer inspection aids derived from preview meshes.
